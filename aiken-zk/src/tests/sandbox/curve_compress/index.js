...
async function convertProofToUncompressed(proof) {
  ...
  const uncompressedProof = {
    "piA:": await compressedG1(proof.pi_a),
...
async function printCompressedProof() {
  console.log("Proof {");
  console.log(JSON.stringify(await convertProofToUncompressed(proof)));
  console.log("}");
...
async function run_program () {
  verificationKey = JSON.parse(fs.readFileSync(args[2], "utf-8"))
